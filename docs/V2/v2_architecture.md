...
当前补充说明：V2 主路径已经交付并完成回归与验收，本文件现在更适合作为“V2 架构基线说明”，后续收口项请结合 [v2_1_plan.md](v2_1_plan.md) 阅读。
...
从当前 chat / hyself / ws 结构看，后端主要瓶颈如下：
...
当前代码已基本按 chat 目标结构收口，后续重点从“是否拆层”转为“query/repository/消费层是否继续细化”。
...
当前资源中心仍保留在 `hyself` 应用内，但已经具备 `Asset` / `AssetReference` 统一抽象，后续建议逐步演进为独立资产域：
...
1. Django 项目包已统一为 `hyself_server.*`
2. chat 的 `interfaces/api`、application、domain、infrastructure 已物理落地
3. 资源中心 payload 组装与部分业务逻辑已下沉到 `hyself/application/services/`
...
当前状态：后端权限边界已经较 V1 明显收口，但 capability 模型仍属于 V2.1 之后的继续演进项。
...
当前状态：以上三项已经落地为现行实现，后续重点转向资源中心接口继续瘦身与资产域独立演进。
...
当前状态：当前实现仍以本地存储为主，对象存储抽象未纳入 V2 发布范围。
...
当前状态：Phase 1 已完成，后续工作转入 [v2_1_plan.md](v2_1_plan.md) 定义的收口与增强阶段。
...
1. 前端 chat 已完成 scene、state、runtime、orchestration 的第二轮拆分，`src/stores/chat.ts` 当前仅保留 assembly store 角色
2. 后端 chat HTTP 接口已物理迁移到 `chat/interfaces/api/`，顶层兼容壳已移除
3. 后端聊天广播事件已收口到通用 `ws/event_bus.py` + `chat/infrastructure/event_bus.py`
4. 前端 realtime 已落到共享 `envelope + dispatcher + domain runtime` 结构，chat/auth/upload/search-audit 都走统一消费层

当前结论：

1. Phase 1 已在 V2.1 完成收口，不再把“前端仍依赖单体 chat store”作为当前状态描述
2. repository 与更细粒度 capability 模型仍有继续演进空间，但属于 V2.2 之后的优化项，而非当前发布阻断
...
3. `UploadedFile` 当前作为兼容层继续存在，但资源中心、聊天附件、头像引用已经在运行时复用统一 asset reference 语义
4. 聊天与资源中心之间的附件复用、另存为资源、资源 realtime 增量事件已经闭环

当前结论：

1. V2.1 在“资产引用复用 + 聊天/资源中心互通”这一层已经收口，可作为当前发布边界
2. 统一 asset picker 入口、上传协议进一步抽象、存量 `UploadedFile` 彻底视图化，统一延后到 V2.2
...
1. V2.1 当前边界已完成图片/文件/聊天记录消息主链路、撤回/删除状态展示、巡检只读补充展示，以及资源中心与聊天附件复用
2. 统一 chat asset picker、更多媒体编辑能力、更完整的消息级回执体系，不纳入当前 V2.1 收口，统一延后到 V2.2
...
1. 错误页、娱乐中心、RBAC、聊天浏览器回归已经形成可重复执行的本地门槛
2. CI 接入和更系统化的 smoke artifact 保留仍属于下一阶段工程化增强
...
1. 第 2、3、4 条已经完成当前阶段收口，chat 接口物理分层、资产引用服务复用、共享 realtime envelope/dispatcher 都已成为现状
2. 第 1 条不再作为当前阻断项：前端保留 assembly store 聚合入口，但 scene/state/runtime/orchestration 已完成稳定拆分
3. 第 5 条按 V2.1 当前边界视为已收口：聊天文件/图片主链路与资源中心复用已完成；统一 picker 与更完整媒体能力转入 V2.2
